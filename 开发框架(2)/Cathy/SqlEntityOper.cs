    ...
    public class SqlEntityOper
    {
        ...
        public string JoneSqlString<T>(T entity, DbType dbType, ref string strConn, ref List<SqlParameter> paralist)
        {
            ...
            foreach (PropertyInfo pi in pis)
            {
                ...
                if (pi.Name != "Conn")
                {
                    if (dbType== DbType.Insert)
                    {
                        //为自增字段时，不计入
                        if (pi.GetCustomAttributes(typeof(Cathy.AutoIncrease), false).Count() != 1)
                        {
                            fieldsList.Add(pi.Name);
                            valuesList.Add("@" + pi.Name);
                            paras.Add(new SqlParameter("@" + pi.Name, pi.GetValue(entity, null) == null ? DBNull.Value : pi.GetValue(entity, null)));
                        }
                    }
                    else
                    {
