...
namespace Cathy.MySql
{
    public static class Communal
    {
        ...
        /// <summary>
        /// 返回格式化的查询条件【字段0>=?p0 and 字段1=?p1 and 字段2  like concat('%',?p2,'%')】
        /// </summary>
        /// <param name="strWhere">where条件 [字段>={值}&字段={值}&字段 like concat('%',?,'%') {值}]</param>
        /// <param name="paras">查询参数</param>
        /// <returns>格式化的查询条件【字段0>=？p0 and 字段1=？p1 and 字段2  like concat('%',?p2,'%')】</returns>
        public static string ToSqlWhere(this string strWhere, ref List<MySqlParameter> paras)
        {
            string result = "";
            if (strWhere != "")
            {
                ...
                foreach (string strFieldValue in strFieldsValue)
                {
                    if (strFieldValue != "")
                    {
                        ...
                        if (str0.Contains("like"))
                        {
                            ...
                            paras.Add(new MySqlParameter("?p" + pIndex, HttpUtility.UrlDecode(str1)));
                        }
                        else
                        {
                            result += " and " + str0.Replace("?", "?p" + pIndex);
            ...
            foreach (string kv in arrKv)
            {
                ...
                if (pv[1] + "" != "")
                {
                    ...
                    if (info != null)
                    {
                        ...
                        object value = HttpUtility.UrlDecode(pv[1] + "");
